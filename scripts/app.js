...
function renderWorkExperience() {
  ...
  cvData.workExperience.forEach((job) => {
    ...
    entry.setAttribute("company", company);
...
function renderEducation() {
  ...
  cvData.education.forEach((edu) => {
    ...
    entry.innerHTML = `
      ...
      <p class="degree-info">${degree} ${"__STATUS_PLACEHOLDER__"}</p>
    `;
    // Normalize status string to camelCase to match translation keys (e.g. "not finished" -> "notFinished")
    const toCamelCase = (s) =>
      String(s)
        .split(/\s+/)
        .map((w, i) =>
...
function renderSkills() {
  ...
  Object.entries(categories).forEach(([categoryKey, skills]) => {
    ...
    skills.forEach((skill) => {
      ...
      const tachometer = createTachometer(skill.level);

      skillItem.appendChild(skillName);
      skillItem.appendChild(tachometer);
...
// Create tachometer-style progress bar
function createTachometer(level) {
  ...
  container.className = "tachometer-rating";
  container.setAttribute("title", t("skillLevels." + label));

  const colors = ["#aaaaaa", "#777777", "#555555", "#333333", "#000000"];
  for (let i = 1; i <= 5; i++) {
    const bar = document.createElement("span");
    bar.className = i <= level ? "tacho-bar filled" : "tacho-bar empty";
    if (i <= level) {
...
function renderLanguages() {
  ...
  cvData.languages.forEach((lang) => {
    ...
    dt.textContent = `${translatedName} — ${level}`;
    container.appendChild(dt);
  });
}

// Render driver license
function renderDriverLicense() {
  const container = document.getElementById("driverLicenseList");
  container.innerHTML = "";

  const license =
    typeof cvData.driverLicense === "object"
      ? cvData.driverLicense[currentLang] || cvData.driverLicense["en"] || ""
      : cvData.driverLicense;

  const dt = document.createElement("dt");
  dt.className = "license-item";
  dt.textContent = license;
  container.appendChild(dt);
}
...
function renderSoftSkills() {
  ...
  const skills =
    typeof cvData.softSkills === "object"
      ? cvData.softSkills[currentLang] || cvData.softSkills["en"] || []
      : cvData.softSkills || [];

  skills.forEach((skill) => {
    const dt = document.createElement("dt");
    dt.className = "soft-skill-item";
    dt.textContent = skill;
    container.appendChild(dt);
  });
}
